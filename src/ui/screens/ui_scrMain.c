...
// Global variables with ui_ prefix
lv_obj_t *ui_wake_group = NULL;
lv_obj_t *ui_sleep_group = NULL;
...
void ui_scrMain_screen_init(void)
{
    ...
    ui_wake_group = lv_obj_create(ui_pnlMain);
    lv_obj_set_size(ui_wake_group, 180, 120);
    lv_obj_align(ui_wake_group, LV_ALIGN_TOP_LEFT, 10, 10);
    lv_obj_set_style_bg_color(ui_wake_group, lv_color_hex(0x535353), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_wake_group, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(ui_wake_group, 15, 0);
    lv_obj_set_style_shadow_width(ui_wake_group, 20, 0);
    lv_obj_set_style_shadow_color(ui_wake_group, lv_color_hex(0x000000), 0);
    lv_obj_set_style_shadow_opa(ui_wake_group, LV_OPA_30, 0);
    lv_obj_set_style_pad_all(ui_wake_group, 15, 0);
    lv_obj_clear_flag(ui_wake_group, LV_OBJ_FLAG_SCROLLABLE);

    // Wake time label with better styling
    lv_obj_t *wake_label = lv_label_create(ui_wake_group);
    lv_label_set_text(wake_label, "Wake Up Time");
    lv_obj_align(wake_label, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_text_color(wake_label, lv_color_hex(0xECF0F1), 0);
    lv_obj_set_style_text_font(wake_label, &lv_font_montserrat_14, 0);

    // Wake time value with prominent display
    ui_lblWakeTime = lv_label_create(ui_wake_group);
    ...
    lv_obj_set_style_text_color(ui_lblWakeTime, lv_color_hex(0xECF0F1), 0);

    // Wake time buttons with better styling
    lv_obj_t *wake_minus = lv_btn_create(ui_wake_group);
    lv_obj_set_size(wake_minus, 40, 40);
    lv_obj_align(wake_minus, LV_ALIGN_BOTTOM_LEFT, 0, 10);
    lv_obj_set_style_bg_color(wake_minus, lv_color_hex(0xE74C3C), 0);
    lv_obj_set_style_radius(wake_minus, 20, 0);
    ...
    lv_obj_t *wake_plus = lv_btn_create(ui_wake_group);
    lv_obj_set_size(wake_plus, 40, 40);
    lv_obj_align(wake_plus, LV_ALIGN_BOTTOM_RIGHT, 0, 10);
    lv_obj_set_style_bg_color(wake_plus, lv_color_hex(0x2ECC71), 0);
    lv_obj_set_style_radius(wake_plus, 20, 0);
    ...
    ui_sleep_group = lv_obj_create(ui_pnlMain);
    lv_obj_set_size(ui_sleep_group, 180, 120);
    lv_obj_align(ui_sleep_group, LV_ALIGN_TOP_RIGHT, -10, 10);
    lv_obj_set_style_bg_color(ui_sleep_group, lv_color_hex(0x535353), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_sleep_group, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(ui_sleep_group, 15, 0);
    lv_obj_set_style_shadow_width(ui_sleep_group, 20, 0);
    lv_obj_set_style_shadow_color(ui_sleep_group, lv_color_hex(0x000000), 0);
    lv_obj_set_style_shadow_opa(ui_sleep_group, LV_OPA_30, 0);
    lv_obj_set_style_pad_all(ui_sleep_group, 15, 0);
    lv_obj_clear_flag(ui_sleep_group, LV_OBJ_FLAG_SCROLLABLE);

    // Sleep time label
    lv_obj_t *sleep_label = lv_label_create(ui_sleep_group);
    lv_label_set_text(sleep_label, "Sleep Time");
    lv_obj_align(sleep_label, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_text_color(sleep_label, lv_color_hex(0xECF0F1), 0);
    lv_obj_set_style_text_font(sleep_label, &lv_font_montserrat_14, 0);

    // Sleep time value
    ui_lblSleepTime = lv_label_create(ui_sleep_group);
    ...
    // Sleep time buttons
    lv_obj_t *sleep_minus = lv_btn_create(ui_sleep_group);
    ...
    lv_obj_t *sleep_plus = lv_btn_create(ui_sleep_group);
